...
int count_ltr(int argc, char **argv, int letter_arg)
{
    ...
    while (argv[1][str_integer]) {
        if (argv[1][str_integer] == argv[letter_arg][0])
            letter_counter++;
        else if (argv[1][str_integer] == argv[letter_arg][0] - 32)
            letter_counter++;
        else if (argv[1][str_integer] == argv[letter_arg][0] + 32)
            letter_counter++;
        str_integer++;
...
int display_freq(int letter_counter, int nbr_ltr)
{
    int unit = 0;
    int hundredth = 0;
    int tenth = 0;
...
int error_input(int argc,char **argv)
{
	...
	if (char_is_ltr(argv[letter_arg][0]) == 0) {
...
int main(int argc, char **argv)
{
    ...
    while (argc != letter_arg) {
        ...
        display_freq(count_ltr(argc, argv, letter_arg), nbr_ltr);
        letter_arg++;
    }
    return (0);
}
